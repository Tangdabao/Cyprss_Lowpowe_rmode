...
#include "LED_Status.h"
    ...
    #define LED_Status_SetP4PinDriveMode(shift, mode)  \
    do { \
        LED_Status_PC =   (LED_Status_PC & \
                                (uint32)(~(uint32)(LED_Status_DRIVE_MODE_IND_MASK << \
                                (LED_Status_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (LED_Status_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define LED_Status_SetP4PinDriveMode(shift, mode)  \
        do { \
            LED_Status_USBIO_CTRL_REG = (LED_Status_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(LED_Status_DRIVE_MODE_IND_MASK << \
                                    (LED_Status_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (LED_Status_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		LED_Status_SetP4PinDriveMode(LED_Status__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(LED_Status_DR & (uint8)(~LED_Status_MASK));
    drVal = (drVal | ((uint8)(value << LED_Status_SHIFT) & LED_Status_MASK));
    LED_Status_DR = (uint32)drVal;
}
...
{
    return (uint8)((LED_Status_PS & LED_Status_MASK) >> LED_Status_SHIFT);
}
...
{
    return (uint8)((LED_Status_DR & LED_Status_MASK) >> LED_Status_SHIFT);
}
...
void LED_Status_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  LED_Status_INTCFG & (uint32)(~(uint32)position);
    LED_Status_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(LED_Status_INTSTAT & LED_Status_MASK);
	LED_Status_INTSTAT = maskedStatus;
    return maskedStatus >> LED_Status_SHIFT;
}
